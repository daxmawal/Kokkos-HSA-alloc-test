...
int main(int argc, char **argv) {
  ...
  for (int step = 0; step < steps; ++step) {
    ...
    float *b = nullptr;
    float *c = nullptr;
    hip_check(hipMallocManaged(&a, bytes), "hipMallocManaged(a)");
    hip_check(hipMallocManaged(&b, bytes), "hipMallocManaged(b)");
    hip_check(hipMallocManaged(&c, bytes), "hipMallocManaged(c)");
